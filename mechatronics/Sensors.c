...
short ReadTapeSensor( enum TapeSensorID p_tsID, enum TapeSensorRetMode p_Mode )
{
  ...
  switch( p_tsID )
  {
    case ts_FRONT_LEFT:
      ...
      if( sTapeSensorReading > TAPE_SENSOR_THRESH_ON  )
      {
    	  sBinaryTapeEvaluation = ts_MDF;  // MDF, field, ok
      }
      else if( sTapeSensorReading < TAPE_SENSOR_THRESH_OFF )
      {
    	  sBinaryTapeEvaluation = ts_TAPE;  // TAPE, border, raise flag
      }
    break;    
  ...
  if( p_Mode == tsrBINARY )
  {
    // High-level return - Bool 1/0 for tape on/off
     return (short) sBinaryTapeEvaluation;     
  }
  else
  {
...
//        Accumulator += (int)ReadIRRangeFinder( rf_FRONT );      
    CurrFrontIRRange = (int)ReadIRRangeFinder( rf_FRONT );      
  //  }
